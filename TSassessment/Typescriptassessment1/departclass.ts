...
export class DepartmentClass implements IDepartment {
    ...
    getDepartmentname(empid: number): string {
        let depid= employees.find((employid) => employid.empid === empid)?.departmentId;
        return  department.find((depart) => depart.id === depid)?.name??" ";
    ...
    getDepartmentnameByfirstname(empname: string): string {
        let employiid = employees.find((employfirstname)=>employfirstname.name.split(" ")[0]===empname)?.departmentId;
        
        return department.find((deptt)=>deptt.id === employiid)?.name??" ";
    ...
    getdepartmentnamebyposition(emppos: string):string{
        let empidd =  employees.find((emppos1)=>emppos1.position===emppos)?.departmentId
            return department.find((dept)=>dept.id===empidd)?.name??" ";
    ...
    getemployeenamebyskill(skill: string): string {
        let deptid = department.find((dept)=>dept.skills?.name===skill)?.id
        return employees.find((emp)=>emp.departmentId===deptid)?.name??" "; //employeename
     ...
     getdepartnamebyskill(skill: string): string {
        return department.find((dept)=>dept.skills?.name===skill)?.name??" "; //department name
